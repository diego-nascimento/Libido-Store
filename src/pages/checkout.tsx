...
import {
  Wrapper, Container, ErrorContainer, ProdutosContainer, BotaoFinalizar, Formulario, Methods, ListMethods, PaymentMethods, CardData, CardInformations, FormularioCard, Aside, ContainerInfoCard, SelectParcelas, SelectEstado, ContainerCep, TotalsInfo
...
const Checkout: React.FC<CarrinhoProps> = ({
  ...
  const {
    register, handleSubmit, formState: { errors }, unregister, setValue, getValues
  } = useForm()
  ...
  const [totalPagar, settotalPagar] = React.useState<number>(total)
  ...
  const [Frete, setFrete] = React.useState<number>(0) // Qual frete foi selecionado, 0 para boleto, 1 para cartao
  ...
  React.useEffect(() => { // calcula o valor atualizado baseado no numero de parcelas
    ...
    } else if (total < 200) { // acima de 200 reais sem juros
  ...
  return (
    <Layout carrinho categorias={categorias}>
      <Head>
                    ...
                    <FormularioCard>
                      ...
                      <ContainerInput borderColor={styles.fontColorInDark} show>
                        <InputMask
                          ...
                          name="CardNumber"
                        />
                        {errors && errors.CardNumber && errors.CardNumber.type === 'required' && (
                        <p>
                          <GoAlert />
                          Esse Campo é Obrigatorio
                        </p>
                      ...
                      <ContainerInfoCard>
                        <ContainerInput borderColor={styles.fontColorInDark} show>
                          <InputMask
                            ...
                            name="CardExpire"
                          />
                          {errors && errors.CardExpire && errors.CardExpire.type === 'required' && (
                          <p>
                            <GoAlert />
                            Esse Campo é Obrigatorio
                          </p>
                      ...
                      <SelectParcelas placeholder="Parcelas" onChange={(e) => handleChangeParcelas(e)}>
                        {Parcelas.map((parcela, index) => (
                          <option
                            ...
                            key={index}
                          >
                            {`${parcela.numero}x - ${parcela.numero === 1 || total > 200
? 'Sem Juros'
                ...
                <TotalsInfo>
                  ...
                  <h2>
                    ...
                    {Intl.NumberFormat('pt-BR', {
                      style: 'currency',
                      currency: 'BRL'
                    }).format((total + Frete) * Parcelas[parcelas - 1].acrescimo / 100)}
                    {' '}
